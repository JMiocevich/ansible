...
alias vmrss="~/personal/ansible/vmrss"
alias f="~/personal/ansible/tmux-sessionizer"
alias k8="kubectl"
# Define a Zsh widget to run your script
run_my_script_widget() {
    BUFFER="~/personal/ansible/tmux-sessionizer"
    zle accept-line
}

# Register the function as a Zsh widget
zle -N run_my_script_widget

# Bind Ctrl+F to the widget
bindkey '^F' run_my_script_widget
...
#
#
# Store the start time before running the command
function preexec() {
    timer=$(date +%s)
}

# Calculate and display the time taken after the command finishes
function precmd() {
    if [[ -n "$timer" ]]; then
        end=$(date +%s)
        elapsed=$((end - timer))
